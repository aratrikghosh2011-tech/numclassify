...
def _explain_odious(n: int) -> str:
    if n < 0:
        return f"{n} < 0, cannot be odious"
    bits = bin(n).count("1")
    return f"{n} in binary = {bin(n)}, popcount = {bits} ({'even' if bits % 2 == 0 else 'odd'}) -> {'evil' if bits % 2 == 0 else 'odious'}"
...
def _explain_carmichael(n: int) -> str:
    if n < 2:
        return f"{n} < 2, cannot be Carmichael"
    if _is_prime(n):
        return f"{n} is prime, Carmichael numbers are composite"
    temp = n
    f = 2
    factors = []
    while f * f <= temp:
        if temp % f == 0:
            ...
            temp //= f
            if temp % f == 0:
                return f"{n} = {' x '.join(map(str, factors))} x ...  --  not squarefree, fails Korselt"
        f += 1
    if temp > 1:
        factors.append(temp)
    if len(factors) < 2:
        return f"{n} is prime power, not Carmichael"
    for p in factors:
        if (n - 1) % (p - 1) != 0:
            return f"{n} fails Korselt: (p-1) = {p-1} does not divide (n-1) = {n-1}"
    return f"{n} = {' x '.join(map(str, factors))}, squarefree, and (p-1)|(n-1) for all p -> Carmichael"
...
def _explain_self_number(n: int) -> str:
    if n < 1:
        return f"{n} < 1, cannot be a self number"
    digits_n = len(str(n))
    for k in range(max(1, n - 9 * digits_n), n):
        if k + _digit_sum(k) == n:
            return f"{n} = {k} + digit_sum({k}) = {k} + {_digit_sum(k)} = {n} -> not a self number (generated by {k})"
    return f"No integer k satisfies k + digit_sum(k) = {n} -> self number"


@register(name="Self Number", category="number theory", oeis="A003052",
          description="Cannot be expressed as k + digit_sum(k) for any k.",
          explain=_explain_self_number)
...
def _explain_keith(n: int) -> str:
    if n < 10:
        return f"{n} has only one digit, cannot be a Keith number"
    digits = [int(d) for d in str(n)]
    seq = list(digits)
    while seq[-1] < n:
        seq.append(sum(seq[-len(digits):]))
    if seq[-1] == n:
        return f"Keith sequence: {', '.join(map(str, seq[:-1]))} -> {n} -> term equals n -> Keith number"
    return f"Keith sequence: {', '.join(map(str, seq))} (exceeds {n}) -> not a Keith number"
